...
use vc_common::types::{MfccSequence, RecognitionResult, WordScore};

use crate::dataset::VocabMap;
use crate::model::{pad_or_truncate, KeywordCnn, KeywordCnnConfig};

/// Inference backend (NdArray without autodiff).
pub type InferBackend = NdArray<f32>;

/// Load a trained model from disk (inference-only).
fn load_model(
