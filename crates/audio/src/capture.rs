...
fn choose_input_config(
    ...
    // Log all supported ranges for diagnostics.
    for c in &configs {
        log::debug!(
    ...
    // Strategy: prefer f32 format, then fewest channels, then closest sample rate.
    // 1) Try to find a config that supports exactly the desired rate.
    // 2) Otherwise, pick the default config from the device.

    let desired_sr = cpal::SampleRate(desired_rate);

    // Sort candidates: prefer f32, then fewer channels, then rate range containing desired.
    ...
    for range in &configs {
        ...
        let mut score: i64 = 0;
        // Prefer f32 format (avoids conversion in callback).
...
/// Automatically negotiates the device's supported config and resamples/downmixes as needed.
...
pub fn record_audio(
    ...
    // We capture raw interleaved samples at the native config, then post-process.
    let native_total =
    ...
    // Wait for recording to complete or cancellation.
    ...
    // Post-process: downmix to mono, then resample to desired rate.
...
/// Returns true if more than 1% of samples are at maximum amplitude.
